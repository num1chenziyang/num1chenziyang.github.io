...
|                                                                                                |
| ---------------------------------------------------------------------------------------------- |
| java: cannot execute: Permission denied                                                        |
| ./ids_install 安装开始报错如题，原因是 java 工具放入/tmp想执行但是其没有执行权限。<br>解决方法见：[/tmp挂载问题](mount命令.md#三./tmp挂载) |

|                                                                                                                             |
| --------------------------------------------------------------------------------------------------------------------------- |
| with an unlicensed version of installanywhere                                                                               |
| ./ids_install 安装开始报错如题，原因是在编译环境搭建时，注册installanywhere处出现问题，重新注册即可（/opt/InstallAnywhere/IA2011/build -ls 10010@172.16.3.213）。 |


## 二、rd_test测试报错
1.执行 sql 语句：truncate table club; 崩溃，查看 AF 文件，发现是在 mt_free 处崩溃。同样版本的 aarch64 没有此问题。
2.随后想编 debug 版调试，但是出现：debug 版不崩溃，release 版稳定复现崩溃的问题。

|      | aarch64    | x86_64                                                                                         |
| ---- | ---------- | ---------------------------------------------------------------------------------------------- |
| 适配过程 | 适配好编译环境，通过 | 适配好编译环境，也进行了代码适配                                                                               |
| 适配内容 | 无          | 1.添加头文件<br># include <sys/types.h><br># include <sys/sysmacros.h><br>2.添加编译参数-std=c++98，-llzma |
| 编译差异 | 编译优化等级为-O2 | 编译优化等级为-O3                                                                                     |

两个解决思路：1.降低编译优化等级（降低到 -O0 解决）。2.调查是否是x86_64的代码适配问题（如将-std=c++98换为-std=c++03 等）。最终，将编译等级调整为 O0 后通过，与 std指定的问题无关。
```shell
vim ids/vobs/tristarm/setup/setup.linux86_64
##修改编译优化等级（这是被允许的操作，以前也发过 O0 的版本）
- OPT_LEVEL=" -O3 "
+ OPT_LEVEL=" -O0 "
```
